...
При запуске backend контейнера автоматически применяются миграции Alembic:
1. Скрипт `entrypoint.sh` ждет готовности базы данных (до 60 секунд)
2. Применяет все миграции командой `alembic upgrade head`
3. Запускает приложение

Миграции применяются **автоматически при каждом запуске контейнера**, поэтому вам не нужно вручную выполнять `alembic upgrade head`.
...
# Применить миграции (автоматически применяются при запуске контейнера)
