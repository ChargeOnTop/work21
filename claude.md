...
# Перезапуск backend (после изменений в коде)
...
**Особенности:**
- Миграции применяются автоматически при запуске (`entrypoint.sh`)
- Код монтируется через volume (hot reload в dev режиме)
- Для применения изменений в `config.py` нужен полный перезапуск (из-за `@lru_cache`)
...
**Критически важно:**
1. **Не генерировать MD файлы** без явного запроса пользователя
2. **Не расписывать мысли** — только код и минимальные пояснения
3. **Минимум комментариев** — только в коде, если необходимо
4. **Краткие ответы** — без лишних объяснений
5. **Фокус на коде** — все токены на написание/исправление кода

**Стиль общения:**
- Прямо к делу
- Без "я сделаю", "сейчас проверю" — просто делай
- Объяснения только если критично для понимания
